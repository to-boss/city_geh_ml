    ...
    pub fn from_gml_with_info(
        ...
        while let Some(info) = sub.next_element()? {
            match (info.namespace.as_str(), info.local_name.as_str()) {
                ...
                (crate::namespace::NS_CITY_FURNITURE, "class") => {
                    class_ = Some(CityFurnitureClassValue(sub.read_text()?));
                }
                (crate::namespace::NS_CITY_FURNITURE, "function") => {
                    function.push(CityFurnitureFunctionValue(sub.read_text()?));
                }
